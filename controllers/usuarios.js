...
function iniciarSesion(req, res, next) {
    ...
    if (!req.body.email || !req.body.password) {
        return res.status(422).json({ errors: { email: "Falta información" } });
    }

    passport.authenticate('local', 
        { session: false },//Este objeto no permite que haya multiples sesiones
        function (err, user, info) {
            ...
            if (user) {
                ...
                return res.status(422).json(info);//Codigo que indica que falta información
            }
        }
    )(req, res, next);
}
